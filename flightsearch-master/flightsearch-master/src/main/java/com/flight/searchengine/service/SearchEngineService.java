...
public class SearchEngineService {
	...
	public List<FlightDetails> getAvailableFlights(String source, String destination, Long departureTime, Integer stops,
			String price, String duration, String flightName, String offercode, String cabin) {
		return repo.findAll(new Specification<FlightDetails>() {
			@Override
			public Predicate toPredicate(Root<FlightDetails> root, CriteriaQuery<?> query,
				...
				if (departureTime != null) {
					predicates.add(criteriaBuilder
							.and(criteriaBuilder.greaterThanOrEqualTo(root.get("departure"), departureTime)));
				}
				...
				if (stops != null) {
					if (stops > 2) {
						predicates.add(criteriaBuilder.and(criteriaBuilder.greaterThanOrEqualTo(root.get("stops"), 0)));
					} else if (stops > 1) {
						predicates.add(criteriaBuilder.and(criteriaBuilder.greaterThanOrEqualTo(root.get("stops"), 2)));
					} else {
